...
const HELIUM_API_KEY = heliumConfig.apiKey;
const TRIGGER_NAME = heliumConfig.trigger;
...
const RETURN_URL = 'heliumbareexample://openapp';
...
export default function App() {
  ...
  const [dontShowIfAlreadyEntitled, setDontShowIfAlreadyEntitled] =
    useState(false);
  ...
  const [allowCheckoutWithoutUserId, setAllowCheckoutWithoutUserId] =
    useState(false);
  ...
  const handlePresent = (triggerName: string) => {
    presentUpsell({
      ...
      onPaywallSkip: event =>
        console.log('[Example] onPaywallSkip fired', event.skipReason),
      onPaywallUnavailable: () =>
        console.log('[Example] onPaywallUnavailable fired'),
    });
  };
  ...
  const handleReset = async () => {
    ...
    eventLog.current = [];
    setEvents([]);
    setAllowWebCheckoutWithoutUserId(false);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        <Text style={styles.title}>Helium Paywall SDK (bare RN)</Text>
        <Text style={styles.subtitle}>
          Platform: {Platform.OS} · API key:{' '}
          {HELIUM_API_KEY === 'REPLACE_ME' ? '(not set)' : 'set'}
          {'\n'}
        ...
        <View style={styles.row}>
          ...
          <Switch
            value={dontShowIfAlreadyEntitled}
            onValueChange={setDontShowIfAlreadyEntitled}
          />
        </View>
        {!isIOS && (
          <View style={styles.row}>
            ...
            <Switch
              value={disableBackNavigation}
              onValueChange={setDisableBackNavigation}
            />
          </View>
        )}
        ...
        <View style={styles.buttons}>
          <Button
            ...
            onPress={() => handlePresent(trigger)}
          />
          <Button
            title="Present invalid trigger (fallback test)"
            onPress={() =>
              handlePresent('nonexistent_trigger_that_does_not_exist')
            }
          />
        </View>
        ...
        <View style={styles.buttons}>
          <Button
            title="Set custom user ID (random UUID)"
            onPress={handleSetCustomUserId}
          />
          <Button title="Get custom user ID" onPress={handleGetCustomUserId} />
          <Button
            title="Clear custom user ID"
            onPress={handleClearCustomUserId}
          />
        ...
        <View style={styles.buttons}>
          <Button
            title="Stub purchases: succeed"
            onPress={() => handleStubPurchase('purchased')}
          />
          <Button
            title="Stub purchases: fail"
            onPress={() => handleStubPurchase('failed')}
          />
          <Button
            title="Clear stubs (real billing)"
            onPress={() => handleStubPurchase(null)}
          />
        </View>
        ...
        {isIOS && (
          <>
            ...
            <View style={styles.buttons}>
              ...
              <Button
                title="Reset Stripe entitlements"
                onPress={() => resetStripeEntitlements()}
              />
              <Button
                title="Reset Paddle entitlements"
                onPress={() => resetPaddleEntitlements()}
              />
            </View>
          </>
        )}
